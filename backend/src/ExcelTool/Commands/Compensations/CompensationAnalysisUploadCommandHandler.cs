...
namespace ExcelTool.Commands.Compensations
{
    public class CompensationAnalysisUploadCommandHandler : IRequestHandler<CompensationAnalysisUploadCommand, MemoryStream>
    {
        ...
        public async Task<MemoryStream> Handle(CompensationAnalysisUploadCommand request, CancellationToken cancellationToken)
        {
            ...
            var dict数据透视 = new Dictionary<string, _赔偿订单统计项>();
            ...
            list赔偿订单.ForEach(it =>
            {
                ...
                if (it._需要ERP操作)
                {
                    var amount = it._对应的ERP操作 == "可售库存弃置" ? it._赔偿数量_总计 : it._赔偿数量_库存;
                    var item = new _赔偿订单统计项(enum处理平台._ERP, it._部门, it._店铺, it._订单号, it.MSKU, it._对应的ERP操作, amount);
                    var refItem = dict数据透视[item.Key];
                    if (refItem != null)
                    {
                        dict数据透视[item.Key].addAmount(item);
                    }
                    else
                    {
                        dict数据透视[item.Key] = item;
                    }
            ...
            var zipPath = Path.Combine(fileSetting.TmpFolder, $"处理结果.zip").Replace(".xlsx", "");
        ...
        private static string _生成统计项的唯一Id(_赔偿订单统计项 item)
        {
            return $"{item._处理平台}{item._部门}{item._店铺}{item._处理动作}{item.MSKU}";
        }
    ...
    class _赔偿订单统计项
    {
        ...
        #region ctor
        private _赔偿订单统计项() { }
        public _赔偿订单统计项(enum处理平台 v处理平台, string v部门, string v店铺, string v单号, string vMSKU, string v处理动作, decimal v数量)
        {
            _处理平台 = v处理平台;
            _部门 = v部门;
            _店铺 = v店铺;
        ...
        public enum处理平台 _处理平台 { get; protected set; }
        ...
        public void addAmount(_赔偿订单统计项 item)
        {
            _所有单号.Add(item._单号);
            _数量 += item._数量;
        }

        //public static _赔偿订单统计项(_赔偿订单 item,)
        //{
    ...
    class _赔偿订单数据透视
    {
        public enum处理平台 _处理平台 { get; set; }
        public string _操作 { get; set; }
